...
class Allowable resource where
  ...
  isAccess :: ActionScope resource -> Access resource -> Bool

instance Allowable App where
  type ActionScope App = App.Privilege
  type Access      App = App.Permission

  isAccess priv@App.Privilege {url} perm@App.Permission {appDomain = Entity _ AppDomain {..}} =
...
instance Allowable Domain where
  ...
  isAccess Domain.Privilege {capability = privCap, ..} perm@Domain.Permission {capability = permCap} =
