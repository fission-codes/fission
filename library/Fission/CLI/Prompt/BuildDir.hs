    ...
    Nothing ->
      guessBuildDir relPath >>= \case
        ...
        Just guess -> do
          ...
          let
            updated = mempty { maybeBuildDir = Just buildDir }

          Env.Override.writeMerge (absPath </> ".fission.yaml") updated
          return buildDir
...
findEnv path = Env.Override.findRecurse (isJust . maybeBuildDir) path >>= \case
  Nothing ->
    return Nothing
...
buildGuesses base = reverse $ fmap (combine base)
