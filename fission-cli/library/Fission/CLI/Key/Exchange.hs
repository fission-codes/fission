module Fission.CLI.Key.Exchange
  ( create
  , forceCreate
  , delete
  , persist
...
delete = exists >>= \case
  False -> return ()
  True  -> removeFile =<< Path.getExchangeKeyPath
...
getAsBytes =
  exists >>= \case
    ...
    True -> do
      publicPath <- Path.getExchangePublicKeyPath
      secretPath <- Path.getExchangeSecretKeyPath
...
exists ::
  ...
  => m Bool
exists = doesFileExist =<< Path.getExchangeKeyPath
