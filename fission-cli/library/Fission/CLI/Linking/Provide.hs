module Fission.CLI.Linking.Provide where -- (listenToLinkRequests) where
--
...
--     reqDID@(DID Key reqPK) <- listenRaw conn
--
...
--         secureBroadcastJSON dmConn key
--         now <- currentTime
--
--         let
--           sessionProof = authenticateSessionKey reqDID machineSK proof key now
--
--         secureBroadcastJSON aesConn sessionProof
--
--         requestorDID <- reattempt 10 $ secureListenJSON aesConn
--         pin          <- reattempt 10 $ secureListenJSON aesConn
--
--         confirmChallenge pin >>= \case
--           False -> do
--             secureBroadcastJSON aesConn Linking.Denied
--             raise Linking.Denied
--
--           True -> do
--             let delegatedUCAN = delegateAllTo requestorDID machineSK proof now
--             secureBroadcastJSON aesConn delegatedUCAN
--
-- confirmChallenge ::
--   ( MonadCleanup m
--   , MonadLogger m
